...
template/
├── integrated/
│   └── ALL_IN_RULE.md         ← 單檔整合套餐（v1.8.0-all-in-one）
└── modular/
    ├── SOUL.md                ← 認知引擎套餐（v13.1.3-deterministic）
    ├── RULE.md                ← 運行合約套餐（v2.8.0-engineering-hardened）
    └── SKILL.md               ← SWDD 框架套餐（v2.6.0-self-evolving）
```
...
| **適用場景** | **其他通用 LLM 工具**（Kilo / Claude Code / Cursor / OpenCode 等）| **openclaw / hermes / omp agent runtime**（經 `installer.py` 部署） |
...
| **適用客群** | 用戶自有的通用 LLM 工具（Kilo / Claude Code / Cursor / OpenCode 等） | **openclaw / hermes / omp agent runtime** 的使用者（由 `installer.py` 部署到 `~/.openclaw/`, `~/.hermes/` 或 `~/.omp/`） |
